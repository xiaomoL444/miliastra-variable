...
const presence = new WeakMap();
export function getPresence(node) {
    return presence.get(node) ?? "present";
}
export function setPresence(node, value) {
    if (value === "present")
        presence.delete(node);
    else
        presence.set(node, value);
}
/**
 ...
 */
export function normalizeNode(workspace, node, expected) {
    ...
    if (effectiveType === "Struct" && isRecord(node.value)) {
        ...
        const expectedId = typeof expectedValue?.structId === "string" ? expectedValue.structId : undefined;
        const actualId = typeof node.value.structId === "string" ? node.value.structId : undefined;
    ...
    const expectedValue = expected && isRecord(expected.value) ? expected.value : undefined;
    const keyType = typeof expectedValue?.key_type === "string"
        ? expectedValue.key_type
        : typeof node.value.key_type === "string"
            ? node.value.key_type
            : undefined;
    const valueType = typeof expectedValue?.value_type === "string"
        ? expectedValue.value_type
        : typeof node.value.value_type === "string"
            ? node.value.value_type
            : undefined;
    const valueStructId = typeof expectedValue?.value_structId === "string"
    ...
    const valueDefault = valueType && (valueType !== "Struct" || (valueStructId && workspace.definitionRef(valueStructId)))
        ? workspace.createDefaultParam(valueType, valueStructId)
        : undefined;
    for (const entry of node.value.value) {
        if (!isRecord(entry))
            continue;
        if (isParamNode(entry.key))
            normalizeNode(workspace, entry.key, keyDefault);
        if (isParamNode(entry.value))
            normalizeNode(workspace, entry.value, valueDefault);
    }
}
